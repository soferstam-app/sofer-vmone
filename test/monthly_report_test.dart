...
void main() {
  ...
  group('time that was never measured', () {
    test('is left out of the average rather than counted as zero', () {
      ...
      expect(day.worked, const Duration(minutes: 60));
      // Six, not three. This asserted three — the very number the comment above
      // it calls wrong — so the test was holding the fault in place. Sixty
      // measured minutes belong to the ten lines that carried them; the other
      // ten say nothing about how fast he writes.
      expect(day.minutesPerLine, closeTo(6, 0.01));
      expect(day.measuredLines, 10);
    });

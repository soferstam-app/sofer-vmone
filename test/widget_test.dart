...
void main() {
  ...
  group('ProductionCalculator – tefillin', () {
    WorkSession session({
      ...
      DateTime? endTime,
        ...
        WorkSession(
          ...
          endTime: endTime ?? DateTime(2026, 1, 1, 10),
          amount: amount,
          startLine: startLine,
          endLine: endLine,
    ...
    test('completed-only variant excludes a partial parshiya', () {
      ...
      expect(
          ProductionCalculator.completedParshiyotInSession(session(
              amount: 1, tefillinType: 'head', parshiya: 1, endLine: 2)),
          isNull);
      // Reaching the last line counts.
      expect(
          ProductionCalculator.completedParshiyotInSession(session(
              amount: 1, tefillinType: 'head', parshiya: 1, endLine: 4)),
          1);
      // Hand parshiya has 7 lines, so line 4 is still partial there.
      expect(
          ProductionCalculator.completedParshiyotInSession(session(
              amount: 1, tefillinType: 'hand', parshiya: 1, endLine: 4)),
          isNull);
    ...
    test('a parshiya resumed in another sitting has one complete average', () {
      final stats = ProductionCalculator.tefillinAverageStats([
        session(
          id: 'first-half',
          amount: 1,
          tefillinType: 'head',
        ...
        session(
          ...
          endTime: DateTime(2026, 1, 1, 10, 25),
        ),
      ]);

      expect(stats.parshiyot, 1);
  ...
  group('IdGenerator', () {
    ...
    test('ids stay roughly sortable by creation time', () {
      ...
      expect(firstMillis, closeTo(DateTime.now().millisecondsSinceEpoch, 5000));
    });
  });
  ...
  group('SessionLogic – validation', () {
    ...
    test('mezuza and tefillin line limits', () {
      ...
      expect(SessionLogic.validateTefillinLine(tefillinType: 'head', line: 4),
          isNull);
      expect(SessionLogic.validateTefillinLine(tefillinType: 'head', line: 5),
          isNotNull);
      // Hand parshiyot allow more lines than head
      expect(SessionLogic.validateTefillinLine(tefillinType: 'hand', line: 7),
          isNull);
    });
  });
  ...
  group('DateLogic', () {
    ...
    test('rollover 0 behaves as the plain calendar date', () {
      expect(
          DateLogic.effectiveDate(
              DateTime(2026, 5, 10, 1, 30), DayStart.midnight),
          DateTime(2026, 5, 10));
      expect(
          DateLogic.effectiveDate(
              DateTime(2026, 5, 10, 23, 0), DayStart.midnight),
          DateTime(2026, 5, 10));
    });
    ...
    test('a late-night session and the day it belongs to agree', () {
      ...
      expect(
          DateLogic.isSameWorkingDay(lateNight, theWorkingDay, at(2)), isTrue);
      // Without the rollover they would be different days
      expect(
          DateLogic.isSameWorkingDay(
              lateNight, theWorkingDay, DayStart.midnight),
          isFalse);
    });

    test('month grouping honours the rollover at a boundary', () {
      final lateNight = DateTime(2026, 5, 1, 1, 0);
      expect(DateLogic.isSameWorkingMonth(lateNight, DateTime(2026, 4), at(3)),
          isTrue);
  ...
  group('ProfitCalculator', () {
    ...
    test('tefillin is priced per pair, not per session amount', () {
      // A whole pair — head and hand.
      expect(
          ProfitCalculator.profit(
              project(ProjectType.tefillin), [s(amount: 1)]),
          80);
      // Head only is one of the two sets in a pair — 52 of its 104 lines.
    ...
    test('a parshiya is worth what it actually holds, not an eighth of a pair',
        () {
      // The correction. Counting parshiyot made all four the same size, so
      // קדש and שמע paid alike; they are 16 and 7 sefer lines of the 104 in a
      // pair, and a writer could not see which of his work paid.
      double paidFor(int parshiya) => ProfitCalculator.profit(
          project(ProjectType.tefillin),
          [s(amount: 1, tefillinType: 'head', parshiya: parshiya)]);

      expect(paidFor(1), closeTo(16 / 104 * 80, 0.001)); // קדש
      expect(paidFor(2), closeTo(12 / 104 * 80, 0.001)); // והיה כי יביאך
      expect(paidFor(3), closeTo(7 / 104 * 80, 0.001)); // שמע
      expect(paidFor(4), closeTo(17 / 104 * 80, 0.001)); // והיה אם שמע

      // The old figure was an eighth of a pair for every one of them.
      expect(paidFor(1), greaterThan(10));
      expect(paidFor(3), lessThan(10));

      // All four together are exactly the set they make.
      expect(paidFor(1) + paidFor(2) + paidFor(3) + paidFor(4),
          closeTo(40, 0.001));
    });
  ...
  group('QuoteCalculator', () {
    ...
    test('non-working days push the completion date out', () {
      ...
      expect(q.workDays, 6);
      expect(q.estimatedCompletion.difference(DateTime(2026, 5, 4)).inDays,
  ...
  group('ExpenseLogic', () {
    ...
    test('categories carry sensible default allocations', () {
      ...
      expect(
          ExpenseLogic.defaultAllocationFor('שונות'), ExpenseAllocation.month);
    ...
    test('a project expense is charged to that project', () {
      ...
      expect(
          ExpenseLogic.totalForProject('p1', expenses)
              .single(Currency.ils)!
              .amount,
          300);
      expect(
          ExpenseLogic.totalForProject('p2', expenses)
              .single(Currency.ils)!
              .amount,
          500);
      expect(
          ExpenseLogic.totalForProject('p3', expenses)
              .single(Currency.ils)!
              .amount,
          0);
    });
    ...
    test('project expenses do not also land in the monthly total', () {
      ...
      expect(
          ExpenseLogic.totalForMonth(DateTime(2026, 5), expenses)
              .single(Currency.ils)!
              .amount,
          0);
    });
    ...
    test('a monthly expense counts in its own month only', () {
      ...
      expect(
          ...
          800);
      expect(
          ExpenseLogic.totalForMonth(DateTime(2026, 6), expenses)
              .single(Currency.ils)!
              .amount,
          0);
    });
    ...
    test('deleted expenses are ignored everywhere', () {
      ...
      expect(
          ExpenseLogic.totalForProject('p1', [deleted])
              .single(Currency.ils)!
              .amount,
          0);
      expect(
          ExpenseLogic.totalForMonth(DateTime(2026, 5), [deleted])
              .single(Currency.ils)!
              .amount,
          0);
    });
    ...
    test('project expenses with no project chosen are flagged', () {
      final orphan = exp(amount: 100, allocation: ExpenseAllocation.project);
    ...
    test('older expenses load as monthly, preserving previous behaviour', () {
      ...
      expect(
          ExpenseLogic.totalForMonth(DateTime(2026, 5), [legacy])
              .single(Currency.ils)!
              .amount,
          200);
    });
  ...
  group('MergeService', () {
    ...
    test('the more recently edited copy wins a conflict', () {
      ...
      final incomingNewer = MergeService.mergeById<Project>([older], [newer]);
      ...
      final incomingOlder = MergeService.mergeById<Project>([newer], [older]);
    ...
    test('a deletion is not undone by an edit made on a stale copy', () {
      ...
      final deleted =
          project('a', updated: DateTime(2026, 6, 1), deleted: true);
    ...
    test('a deletion survives however long a device was away', () {
      ...
      final r = MergeService.mergeById<Project>([longGone], [staleLiveCopy]);
  ...
  group('the working day is frozen when a session is recorded', () {
    ...
    test('changing the boundary later does not re-file old work', () {
      ...
      expect(
          DateLogic.sessionIsOnDay(
              old, DateTime(2026, 5, 10), DayStart.midnight),
          isTrue);
      expect(
          DateLogic.sessionIsOnDay(
              old, DateTime(2026, 5, 11), DayStart.midnight),
          isFalse);
    });
    ...
    test('month grouping follows the frozen day across a month boundary', () {
      ...
      expect(DateLogic.sessionIsInMonth(s, firstOfElul, DayStart.midnight),
          isFalse,
  ...
  group('forward compatibility of saved records', () {
    test('a field from a newer version survives a round trip', () {
      ...
      expect(written['nestedFutureField'], {
        'a': 1,
        'b': [2, 3]
      });
  ...
  group('HebrewWorkCalendar — configurable categories', () {
    ...
    test('Friday is half a day or nothing, never a full one', () {
      ...
      expect(
          WorkCalendarRules.fromJson(
              const {'schemaVersion': 2, 'friday': 'full'}).friday,
          DayWeight.none);
    });
    ...
    test('the days between Yom Kippur and Sukkot', () {
      ...
      for (var d = 11; d <= 13; d++) {
        ...
        expect(
            ...
            anyOf(
                NonWorkReason.betweenYomKippurAndSukkot, NonWorkReason.shabbat),
            reason: '$d Tishrei');
      }
    });
    ...
    test('the most restrictive setting wins where two overlap', () {
      ...
      WorkDay withRules(DayWeight fri, DayWeight chan) =>
          HebrewWorkCalendar.classify(
              friday!, shabbatOnly.copyWith(friday: fri, chanukah: chan));
  ...
  group('the rule is frozen, not the day it produced', () {
    ...
    test('a session recorded before rules were kept uses its frozen day', () {
      final legacy =
          ruled(justAfterMidnight, null, frozenDay: DateTime(2026, 5, 10));
  ...
  group('QuoteExpense', () {
    test('a cost per unit is the same whatever the quantity', () {
      const parchment = QuoteExpense(label: 'קלף', amount: 45, perUnit: true);
    ...
    test('a cost for the whole job is spread over the quantity', () {
      const delivery = QuoteExpense(label: 'משלוח', amount: 60, perUnit: false);

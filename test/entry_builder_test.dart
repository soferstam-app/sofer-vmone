...
void main() {
  ...
  EntryOutcome build(EntryInput input,
  ...
  group('a sefer, one page', () {
    ...
    test('refuses a page past the end of the book', () {
      expect(
          rejection(build(sefer(
              pageFrom: '300', lineFrom: '1', lineTo: '42', totalPages: 245))),
          contains('245'));
    });

    test('refuses lines that do not fit the page', () {
      expect(
          rejection(build(sefer(pageFrom: '5', lineFrom: '1', lineTo: '50'))),
          contains('42'));
      expect(
          rejection(build(sefer(pageFrom: '5', lineFrom: '20', lineTo: '3'))),
          isNotEmpty);
  ...
  group('a sefer, a range of pages', () {
    ...
    test('uses the line fields as the two ends of one continuous range', () {
      // The reported real-world case: page מג through page מד, ending at line
      // 21, is one and a half pages — not two full pages.
      ...
      expect(result.sessions.map((s) => s.amount), [43, 44]);
      expect(result.sessions.map((s) => s.startLine), [1, 1]);
      expect(result.sessions.map((s) => s.endLine), [42, 21]);
      expect(result.reachedPage, 44);
      expect(result.reachedLine, 21);

      expect(result.sessions.first.duration, const Duration(hours: 2));
      expect(result.sessions.last.duration, const Duration(hours: 1));
    });

    test('supports partial first and last pages with full pages between', () {
      final result = built(build(sefer(
        pageFrom: '43',
        pageTo: '45',
        lineFrom: '30',
      ...
      expect(result.sessions.map((s) => s.endLine), [42, 42, 10]);
    });

    test('refuses an invalid endpoint instead of silently making full pages',
        () {
      expect(
          rejection(build(sefer(
              pageFrom: '43', pageTo: '44', lineFrom: '1', lineTo: '50'))),
          contains('42'));
    });
    ...
    test('refuses an end page below the start', () {
      expect(
          rejection(build(
    ...
    test('refuses an unreadable end page instead of ignoring it', () {
      expect(
          rejection(build(sefer(
              pageFrom: '43',
              pageTo: 'not-a-page',
  ...
  group('tefillin', () {
    ...
    test('a single parshiya names itself', () {
      final result = built(build(tefillin(mode: 'parshiya')));
      final s = result.sessions.single;
      expect(s.amount, 1);
      expect(s.parshiya, 1);
      expect(s.tefillinType, 'head');
      expect(s.pairIndex, 1);
      expect(s.description, contains('קדש'));
    });
    ...
    test('a later parshiya is refused until the previous one is finished', () {
      expect(
        rejection(build(tefillin(mode: 'parshiya', parshiya: 2))),
        contains('קדש'),
      );

      final previous = WorkSession(
        id: 'kadesh',
        projectId: 'p',
        startTime: start,
        endTime: end,
      ...
      expect(
        build(tefillin(mode: 'parshiya', parshiya: 2), history: [previous]),
        isA<EntryBuilt>(),
      );
    });
  });

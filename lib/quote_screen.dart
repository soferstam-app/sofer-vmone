...
class _QuoteScreenState extends State<QuoteScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final estimate = pace == null
        ...
        : QuoteCalculator.estimate(
            units: double.tryParse(_unitsCtrl.text) ?? 0,
            timePerUnit: pace,
            targetHourlyRate: double.tryParse(_rateCtrl.text) ?? 0,
            hoursPerDay: double.tryParse(_hoursCtrl.text) ?? 0,
            expensesPerUnit: double.tryParse(_expensesCtrl.text) ?? 0,
  ...
  Widget _resultCard(QuoteEstimate e) {
    return Card(
      elevation: 3,
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          ...
          children: [
            ...
            const Divider(),
            _row("מחיר ליחידה:", "₪${e.pricePerUnit.toStringAsFixed(2)}"),
            Padding(
              padding: const EdgeInsets.only(top: 6),
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  const Text("מחיר מוצע:",
                      style: TextStyle(

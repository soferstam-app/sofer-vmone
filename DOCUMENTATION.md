...
### `Expense`
`id`, `product` (קטגוריה/מוצר), `date`, `amount`. אין `isDeleted` – מחיקה היא פיזית מהרשימה.
...
2. **מיזוג (`_mergeLists`)** לפי `id`: הרשומה עם `lastUpdated` מאוחר יותר מנצחת (בהוצאות משתמשים ב-`date` כמפתח ההשוואה).
3. **ניקוי (`_purgeOldDeleted`)**: פריטים עם `isDeleted == true` שעברו 30 יום נמחקים לצמיתות (הוצאות אינן עוברות ניקוי).
4. שמירה מקומית + העלאה חזרה לדרייב (update אם קיים, create אם לא).

⚠️ כל השגיאות נבלעות ב-`try/catch` עם `debugPrint` בלבד – כישלון סנכרון שקוף למשתמש.
...
קבצים לא מנוהלים בתיקייה: `sofer_vmone_0.3.0_windows.zip` (14MB), `סופר ומונה 0.3.0.apk` (57MB) – **לא רצוי לקמט אותם למאגר**; מקומם ב-GitHub Releases.
...
| 4 | שגיאות סנכרון נבלעות | `syncData()` לא מדווח למשתמש על כישלון |
| 5 | `history_screen.dart` ריק | שריד – ניתן למחוק |
| 6 | `test/widget_test.dart` | טסט ברירת מחדל שאינו בודק דבר בפרויקט; אין כיסוי טסטים ללוגיקה העסקית (חישובי הספק/רווח הם מועמדים מובהקים) |

...
| רכיב | תפקיד |
|---|---|
| `NonWorkReason` | הסיבה שיום אינו יום עבודה, עם תווית בעברית לתצוגה |
| `FridayWork` | `none` / `half` / `full` – כמה נכתב ביום שישי |
| `WorkCalendarRules` | אילו ימים המשתמש מגדיר כימי הפסקה + `inIsrael`; `toJson`/`fromJson` לאחסון |
| `classify(jc, rules)` | מסווג יום → `WorkDay(value, reason)` |
| `classifyHebrewDate(y, m, d, rules)` | סיווג ישירות מתאריך עברי, בלי מעבר בלועזי כלל (משמש בבדיקות) |
...
| `daysOff(from, to, rules)` | כל ימי ההפסקה בטווח, לתצוגת "למה זה כל כך רחוק" |
| `nextWorkDay(from, rules)` | יום העבודה הבא |

**סדר הסיווג חשוב:** קודם נקבעות סיבות הלוח העברי ורק אחר כך היום בשבוע. יום שישי שהוא גם חול המועד חייב להיות סגור גם למי שמוגדר לו "שישי יום מלא", ומוצאי שבת שעדיין יום טוב לא ייזקף כחצי יום.

**ימים שאינם ניתנים לכיבוי:** שבת, יום טוב ותשעה באב.

**ברירות המחדל** (ניתנות לשינוי במסך "ימי עבודה"): שישי סגור · חול המועד · ערבי חג · צומות · ערב תשעה באב · י״א–י״ד תשרי · ח׳–י״ד ניסן · פורים ושושן פורים · חנוכה. **פתוחים כברירת מחדל:** ראש חודש, ט״ו בשבט, ל״ג בעומר, פסח שני, ט״ו באב, איסרו חג.
